...
namespace OccultistArcanist.Engine.WizardSchools {
    public class EnchantmentSchool : WizardSchool
    {
        ...
        public EnchantmentSchool() : base()
        {

        }
        public override void initializeBaseFeatures()
        {
            school_used = SpellSchool.Enchantment;
            wizard_school_progression = BlueprintTools.GetBlueprint<BlueprintProgression>("252363458703f144788af49ef04d0803");
            first_base_feature = BlueprintTools.GetBlueprint<BlueprintFeature>("9e4c4799735ae9c45964e9113107ef02");
            second_base_feature = BlueprintTools.GetBlueprint<BlueprintFeature>("972ed7d4b0e497345a69b06afc052f67");
        }
        ...
        public override void createFirstSchoolPower()
        {
            var copied_feature = first_base_feature.CreateCopy(Main.HaddeqiModContext, guid_used + first_base_feature.name, bp => {
                ...
                var new_ability = old_ability.CreateCopy(Main.HaddeqiModContext, guid_used + old_ability.name, bl => {
                    ...
                    var old_ability_2 = bl.GetComponent<AbilityEffectStickyTouch>().TouchDeliveryAbility;
                    var new_ability_2 = old_ability_2.CreateCopy(Main.HaddeqiModContext, guid_used + old_ability_2.name, bk => {
                        bk.GetComponent<ContextRankConfig>().m_Class[0] = school_class.ToReference<BlueprintCharacterClassReference>();
                    });
                    bl.GetComponent<AbilityEffectStickyTouch>().m_TouchDeliveryAbility = new_ability_2.ToReference<BlueprintAbilityReference>();
                });
